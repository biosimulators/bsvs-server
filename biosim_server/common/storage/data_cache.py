...
async def get_cached_omex_file(uploaded_file: UploadFile) -> OmexFile:

    contents = await uploaded_file.read()
    file_hash_md5: str = hashlib.md5(contents).hexdigest()
    ...
    if omex_file is None:
        ...
        filename = Path(uploaded_file.filename or (uuid.uuid4().hex + ".omex")).name
        ...
        full_gcs_path: str = await file_service.upload_bytes(file_contents=contents, gcs_path=gcs_path)
        ...
        omex_file = OmexFile(file_hash_md5=file_hash_md5, omex_gcs_path=full_gcs_path, uploaded_filename=filename,
                             bucket_name=get_settings().storage_bucket, file_size=len(contents))
